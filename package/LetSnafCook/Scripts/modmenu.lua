...
-- API VERSION 3  (a plain integer, +1 on each additive spec change: v1 = original, v2 = item
-- `desc`, v3 = requestRefresh).
--
...
local K_REFRESH = PFX .. "refresh" -- integer stamp; requestRefresh bumps, the menu re-reads on change
...
-- Ask the menu to re-read the published store while its view is open (a dynamic menu whose
-- action added/removed items). Call it AFTER register() has republished the new schema, so the
-- store is already consistent when the menu re-reads. The refresh is GLOBAL (the menu re-reads
-- every registered mod), hence no parameter. The menu watches one scalar while open, so the
-- refresh lands within a beat; when the menu is closed (or not installed) this is inert, and
-- the next open picks up the new spec anyway. Feature-detect on older bridges:
--   if modmenu.requestRefresh then modmenu.requestRefresh() end
function M.requestRefresh()
    return sset(K_REFRESH, (tonumber(sget(K_REFRESH)) or 0) + 1)
end
...
-- the current refresh stamp (0 when never requested). The menu re-reads the store on change.
function M.refreshStamp()
    return tonumber(sget(K_REFRESH)) or 0
end
